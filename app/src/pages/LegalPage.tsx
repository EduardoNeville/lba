...
function useLegalData() {
  const { lang } = useLang()
  if (lang === 'fr') return { hero: { ...hero, ...fr.hero }, pillars: fr.pillars, areas: fr.areas, practice: { ...practice, ...fr.practice, image: practice.image }, cta: { ...cta, ...fr.cta, image: cta.image } }
  if (lang === 'es') return { hero: { ...hero, ...es.hero }, pillars: es.pillars, areas: es.areas, practice: { ...practice, ...es.practice, image: practice.image }, cta: { ...cta, ...es.cta, image: cta.image } }
...
export function LegalPage() {
  ...
  return (
    <>
      ...
      <CrossLinkBand
        left={
          <div>
            ...
            <h2 className="font-display mt-3 text-2xl uppercase leading-snug">{practice.heading}</h2>
        ...
        center={
          <div aria-hidden className="hidden overflow-hidden lg:block">
            <img src={practice.image} alt={practice.alt} className="h-full w-full object-cover" />
          </div>
        }
        right={
          <div>
      ...
      />
      <CtaBand heading={cta.heading} subline={cta.subline} image={cta.image} cta={{ to: '/inquiry', label: 'Make a private enquiry' }} />
    </>
  )
}
